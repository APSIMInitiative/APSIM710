...
namespace EBMath
{
    ...
    /// </summary>
    /// 
    public enum Type { Variable, Value, Operator, EvalFunction, Result, Bracket, Comma, Error }
    ...
    public class ExpressionEvaluator
    {
                ...
                switch (state)
                {
                    ...
                    case 3:
                        ...
                        else
                        {
                            ...
                            ctSymbol.m_value = 0;
                            if (equation[i] == '(')
                                ctSymbol.m_type = Type.EvalFunction;
                            else
                            {
        ...
        public void Infix2Postfix()
        {
            ...
            foreach (Symbol sym in m_equation)
            {
                ...
                else
                {
                    ...
                    {
                        tpSym = (Symbol)tpStack.Pop();
                        while ((tpStack.Count != 0) && ((tpSym.m_type == Type.Operator) || (tpSym.m_type == Type.EvalFunction) || (tpSym.m_type == Type.Comma)) && (Precedence(tpSym) >= Precedence(sym)))
                        {
                            m_postfix.Add(tpSym);
                            tpSym = (Symbol)tpStack.Pop();
                        }
                        if (((tpSym.m_type == Type.Operator) || (tpSym.m_type == Type.EvalFunction) || (tpSym.m_type == Type.Comma)) && (Precedence(tpSym) >= Precedence(sym)))
                            m_postfix.Add(tpSym);
                        else
                            tpStack.Push(tpSym);
                    }
        ...
        public void EvaluatePostfix()
        {
            ...
            foreach (Symbol sym in m_postfix)
            {
                ...
                else if (sym.m_type == Type.EvalFunction)
                {
                    fnParam.Clear();
                    tpSym1 = (Symbol)tpStack.Pop();
