...
class GenericArbitratorXY
{
    [Param]
    string[] PartitionOrgans = null;

    [Param]
    string[] PartitionRules = null;
    ...
    internal void PartitionDM(List<Organ1> Organs)
    {
        ...
        if (!MathUtility.FloatsAreEqual(dlt_dm_green_tot, DMSupply, 1.0E-4f))
        {
    ...
    internal void RetranslocateDM(List<Organ1> Organs)
    {
        ...
        double dlt_dm_green_retrans = dlt_dm_retrans_to_fruit * MathUtility.Divide(dm_demand_differential, demand_differential_begin, 0.0);
       ...
       if (!MathUtility.FloatsAreEqual(FinalRetranslocation, dlt_dm_green_retrans))
          {
    ...
    {
        Function F = My.LinkByName("FracDMRemainingIn" + OrganName) as Function;
        if (F == null)
    ...
    internal double DoNPartition(double n_fix_pot, List<Organ1> Organs)
    {
        ...
        foreach (Organ1 Organ in Organs)
        {
            if (n_excess > 0.0)
            {
                double plant_part_fract = MathUtility.Divide(Organ.NCapacity, NCapacityTotal, 0.0);
                Organ.DoNPartition(Organ.NDemand + n_excess * plant_part_fract);
            }
            else
            {
                double plant_part_fract = MathUtility.Divide(Organ.NDemand, NDemandTotal, 0.0);
        ...
        if (!MathUtility.FloatsAreEqual(GrowthNTotal - nUptakeSum, 0.0))
        {
        ...
        double NFixDemandTotal = MathUtility.Constrain(NDemandTotal - nUptakeSum, 0.0, double.MaxValue); // total demand for N fixation (g/m^2)
        double NFixUptake = MathUtility.Constrain(n_fix_pot, 0.0, NFixDemandTotal);
        ...
        NFixUptake = NFixUptake * MathUtility.Divide(n_demand_differential, NFixDemandTotal, 0.0);
        foreach (Organ1 Organ in Organs)
    ...
    internal void doNSenescedRetranslocation(List<Organ1> Organs)
    {
        ...
        navail = MathUtility.Constrain(navail, 0.0, n_demand_tot);
