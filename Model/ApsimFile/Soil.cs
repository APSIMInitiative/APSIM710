...
namespace ApsimFile
{
    ...
    public class Soil
    {
        public class Variable
        {
            ...
            /// <summary>
            /// Modify the Values and ValueThickness arrays to cover the range of ToThickness
            ///  e.g. IF ValueThickness	    Values                  ToThickness   
            ///                0-100          10                       0-100
            ///              100-600          11                     100-250
            ///                                                      250-500
            ///                                                      500-750
            ...
            ///                                                     1500-1800
            ///
            /// The arrays that is returned will cover the range down to at least 1800. e.g.
            ///   ValueThickness	    Values      
            ///         0-100          10        
            ///       100-600          11        
            ///       600-1100         9
            ///      1100-1700         5
            ...
            private static void FillValuesDownToSoilDepth(string VariableName, string Units, ref double[] Values, ref double[] ValueThickness, double[] ToThickness, XmlNode SoilNode)
            {
                ...
                List<double> NewThickness = new List<double>();
                for (int i = 0; i < Values.Length; i++)
                    if (Values[i] != MathUtility.MissingValue)
                    {
                        NewValues.Add(Values[i]);
                        NewThickness.Add(ValueThickness[i]);
                    }
                
                if (VariableName.ToLower() == "sw")
                {
                    int LastIndex = Values.Length - 1;

                    // Get the new thicknesses right.
                    double LastThickness = ValueThickness[LastIndex];
                    ...
                    NewValues.Add(0);  // This will be constrained later to cropLL.
                         ...
                         VariableName.ToLower() == "ll" || VariableName.ToLower() == "ll15" ||
                ...
                {
                    XmlNode Node = FindVariableParentOptional(SoilNode, VariableName, false, Find.NonSampleOnly);
                    Soil.Variable Var = null;
                    if (Node != null)
                        Var = Soil.GetOptionalFromProfileNode(SoilNode, Node, VariableName);
                    if (Var == null)
                        throw new Exception("Cannot find variable: " + VariableName + " in soil");
                    Var.Units = Units;
                ...
                ValueThickness = NewThickness.ToArray();
            }
            ...
            /// </summary>
            private static void InFillValues(List<double> SampleValues, List<double> SampleThickness,
                                                         double[] SoilValues, double[] SoilThickness)
            {
                ...
                for (int SoilLayer = 0; SoilLayer < SoilThickness.Length; SoilLayer++)
                {
                    ...
                    {
                        SampleThickness.Add(CumSoilDepth - CumSampleDepth);
                        if (SoilValues[SoilLayer] == MathUtility.MissingValue)
                            SampleValues.Add(0.0);
                        else
                            SampleValues.Add(SoilValues[SoilLayer]);
        ...
        public static Soil.Variable GetOptional(XmlNode SoilNode, string VariableName)
        {
            ...
            if (Var == null)
            {
                XmlNode ParentNode = FindVariableParentOptional(SoilNode, VariableName, false, Find.PreferSample);
                if (ParentNode == null)
                    return null;
                else
                    return new Soil.Variable(ParentNode, VariableName);
        ...
        /// </summary>
        public enum Find { NonSampleOnly, PreferSample };
        public static XmlNode FindVariableParentOptional(XmlNode SoilNode, string VariableName, bool CreateCropVariablesIfNecessary, Find FindType)
        {
            ...
            // Go through all nodes and try and find a sample node. If found then use it and get
            // rid of the others.
            for (int i = 0; i < ParentNodes.Count; i++)
            {
                if (FindType == Find.PreferSample && ParentNodes[i].Name == "Sample")
                {
                    XmlNode SampleNode = ParentNodes[i];
                    ParentNodes.Clear();
                    ParentNodes.Add(SampleNode);
                }
                else if (FindType == Find.NonSampleOnly && ParentNodes[i].Name == "Sample")
                {
        ...
        private static string CheckProfile(XmlNode SoilNode)
        {
            ...
            if (oc.ThicknessMM.Length == 0)
                throw new Exception("Cannot find OC values in soil");

            for (int layer = 0; layer != oc.ThicknessMM.Length; layer++)
            {
                int RealLayerNumber = layer + 1;
