...
public partial class SoilNitrogen
{
	...
	partial class soilCNPatch
	{
		...
		public void DecomposeResidues()
		{
			...
			else
			{
				...
				if (SumDoubleArray(g.pot_c_decomp) > g.epsilon)
				{
					...
					double NDemand = MathUtility.Divide(SumDoubleArray(dltC_into_biom), g.MBiomassCNr, 0.0) +
									  MathUtility.Divide(SumDoubleArray(dltC_into_hum), g.HumusCNr, 0.0);
					...
					double NAvailable = MineralNAvailable + SumDoubleArray(g.pot_n_decomp);

					// 2.3.3. factor to reduce mineralization rate if insufficient N is available
					double ReductionFactor = 1.0;
					if (NDemand > NAvailable)
					{
						ReductionFactor = MathUtility.Divide(MineralNAvailable, NDemand - SumDoubleArray(g.pot_n_decomp), 0.0);
			...
			if (SumDoubleArray(dlt_c_res_to_biom) + SumDoubleArray(dlt_c_res_to_hum) >= g.epsilon)
			{
		...
		{
			int nLayers = g.dlayer.Length;			// number of layers in the soil
			if (SumDoubleArray(hum_c) >= g.epsilon)
			{
			...
			if (SumDoubleArray(biom_c) >= g.epsilon)
			{
			...
			if ((SumDoubleArray(fom_c[0]) + SumDoubleArray(fom_c[1]) + SumDoubleArray(fom_c[2])) >= g.epsilon)
			{
			...
			if (SumDoubleArray(hum_c) + SumDoubleArray(biom_c) + SumDoubleArray(fom_c[0]) + SumDoubleArray(fom_c[1]) + SumDoubleArray(fom_c[2]) >= g.epsilon)
			{
		...
		{
			int nLayers = g.dlayer.Length;			// number of layers in the soil
			if (SumDoubleArray(urea) >= g.epsilon)
			{
		...
		{
			int nLayers = g.dlayer.Length;			// number of layers in the soil
			if (SumDoubleArray(no3) >= g.epsilon)
			{
		...
		{
			int nLayers = g.dlayer.Length;			// number of layers in the soil
			if (SumDoubleArray(nh4) >= g.epsilon)
			{
