	...
	class soilCNPatch
	{
		...
		public void Process()
		{
			...
			if (g.is_pond_active)
			{
				...
				hum_n[0] = MathUtility.Divide(hum_c[0], g.hum_cn, 0.0);
				biom_n[0] = MathUtility.Divide(biom_c[0], g.biom_cn, 0.0);
			}
			else
			{
				// Decompose residues
				...
				for (int layer = 0; layer < nLayers; layer++)
				{
					hum_n[layer] = MathUtility.Divide(hum_c[layer], g.hum_cn, 0.0);
					biom_n[layer] = MathUtility.Divide(biom_c[layer], g.biom_cn, 0.0);
		...
		private void DecomposeResidues()
		{
			...
			double n_demand = MathUtility.Divide(SumDoubleArray(pot_c_decomp) * g.ef_res * g.fr_res_biom, g.biom_cn, 0.0) +
							  MathUtility.Divide(SumDoubleArray(pot_c_decomp) * g.ef_res * (1.0 - g.fr_res_biom), g.hum_cn, 0.0);
		...
		private void MineraliseHumus(int layer)
		{
			...
			double dlt_n_min_hum = MathUtility.Divide(dlt_c_min_hum, g.hum_cn, 0.0);
			...
			dlt_n_hum_2_min[layer] = dlt_n_min_hum - MathUtility.Divide(dlt_c_hum_2_biom[layer], g.biom_cn, 0.0);
		}
		...
		private void MineraliseBiomass(int layer)
		{
			...
			dlt_n_biom_2_min[layer] = dlt_n_min_biom - MathUtility.Divide(dlt_c_biom_2_hum[layer], g.hum_cn, 0.0) - MathUtility.Divide((dlt_c_min_biom - dlt_c_biom_2_atm[layer] - dlt_c_biom_2_hum[layer]), g.biom_cn, 0.0);
		}
		...
		private void MineraliseFOM(int layer, out double[] dlt_c_biom, out double[] dlt_c_hum, out double[] dlt_c_atm, out double[] dlt_fom_n, out double dlt_n_min)
		{
			...
			if (fomC >= g.fom_min)
			{
				...
				double n_demand = MathUtility.Divide(dlt_c_biom_tot, g.biom_cn, 0.0) + MathUtility.Divide(dlt_c_hum_tot, g.hum_cn, 0.0);
		...
		private double UreaHydrolysis_ApsimSoilN(int layer)
		{
			...
			else
			{
				...
				// get the soil temperature factor
				double stf = Math.Max(0.0, Math.Min(1.0, (g.st[layer] / 40.0) + 0.20));
		...
		private double Nitrification_ApsimSoilN(int layer)
		{
			...
			double opt_nitrif_rate_ppm = MathUtility.Divide(g.nitrification_pot * nh4_ppm, nh4_ppm + g.nh4_at_half_pot, 0.0);
		...
		private double Denitrification_ApsimSoilN(int layer)
		{
			...
			// get the soil temperature factor
			double stf = Math.Max(0.0, Math.Min(1.0, 0.1 * Math.Exp(0.046 * g.st[layer])));
		...
		private double Denitrification_Nratio(int layer)
		{
			...
			double RtermC = 0.1;
			bool didInterpolate;
			double RtermD = MathUtility.LinearInterpReal(WFPS, g.dnit_wfps, g.dnit_n2o_factor, out didInterpolate);
		...
		private double Nitrification_WNMM(int layer)
		{
			...
			tf = Math.Max(0.0, 0.41 * (g.st[layer] - 5.0) / 10.0);
			...
			// n2o
			double fTemp = 0.1 + 0.9 * (g.st[layer] / (g.st[layer] + Math.Exp(9.93 - 0.312 * g.st[layer])));
		...
		private double Denitrification_NEMIS(int layer)
		{
			...
			tf = 1.0;
			if (g.st[layer] < 11.0)
				tf = Math.Exp(0.1 * (g.st[layer] - 11.0) * Math.Log(89, Math.E) - 9 * Math.Log(2.1, Math.E));
			else //st >= 11
				tf = Math.Exp(0.1 * (g.st[layer] - 20.0) * Math.Log(2.1, Math.E));
		...
		private double Denitrification_WNMM(int layer)
		{
			...
			// soil temperature effects (0-1)       
			tf = 0.1 + 0.9 * g.st[layer] / (g.st[layer] + Math.Exp(9.93 - 0.312 * g.st[layer]));
		...
		private double pHFNitrf(int layer)
		{
			...
			bool DidInterpolate;
			return MathUtility.LinearInterpReal(g.ph[layer], g.pHf_nit_pH, g.pHf_nit_values, out DidInterpolate);
		}
		...
		private double WFNitrf(int layer, int index)
		{
			...
			else
			{
				...
				wfd = MathUtility.Divide(g.sw_dep[layer] - g.ll15_dep[layer], g.dul_dep[layer] - g.ll15_dep[layer], 0.0);
				wfd = Math.Max(0.0, Math.Min(1.0, wfd));
			}

			bool didInterpolate;
			if (index == 1)
				return MathUtility.LinearInterpReal(wfd, g.wfnit_index, g.wfnit_values, out didInterpolate);
			else
		...
		private double WFDenit(int layer)
		{
			...
			if (g.sw_dep[layer] > g.dul_dep[layer])  // saturated
				wfd = Math.Pow(MathUtility.Divide(g.sw_dep[layer] - g.dul_dep[layer], g.sat_dep[layer] - g.dul_dep[layer], 0.0), g.dnit_wf_power);
		...
		private double WF(int layer, int index)
		{
			...
			{ // saturated
				wfd = Math.Max(1.0, Math.Min(2.0, 1.0 +
					MathUtility.Divide(g.sw_dep[layer] - g.dul_dep[layer], g.sat_dep[layer] - g.dul_dep[layer], 0.0)));
			}
			else
			{ // unsaturated
				// assumes rate of mineralisation is at optimum rate until soil moisture midway between dul and ll15
				wfd = Math.Max(0.0, Math.Min(1.0, MathUtility.Divide(g.sw_dep[layer] - g.ll15_dep[layer], g.dul_dep[layer] - g.ll15_dep[layer], 0.0)));
			}

			if (index == 1)
			{
				bool didInterpolate;
				return MathUtility.LinearInterpReal(wfd, g.wfmin_index, g.wfmin_values, out didInterpolate);
			}
		...
		private double TF(int layer, int index)
		{
			...
			if (g.st[layer] > 0.0)
				return Math.Max(0.0, Math.Min(1.0, MathUtility.Divide(g.st[layer] * g.st[layer], g.opt_temp[index - 1] * g.opt_temp[index - 1], 0.0)));
			else
		...
		private double RothcTF(int layer, int index)
		{
			...
			double t = Math.Min(g.st[layer], g.opt_temp[index - 1]);
		...
		private double SoilTempFactor(int layer, int index, BendingStickData Parameters)
		{
			...
			return Math.Min(1.0, Math.Pow(beta * Math.Max(0.0, g.st[layer] - Tzero), CurveN));
		}
		...
		private double convFactor_kgha2ppm(int layer)
		{
			...
			return MathUtility.Divide(100.0, g.bd[layer] * g.dlayer[layer], 0.0);
		}
