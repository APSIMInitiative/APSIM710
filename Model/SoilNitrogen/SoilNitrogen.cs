...
public class SoilNitrogen
{
    ...
    private double[] ureappm
    {
        get
        {
            ...
            if (initDone)
            {
                ...
                for (int layer = 0; layer < dlayer.Length; ++layer)
                    result[layer] = _urea[layer] * convFactor_kgha2ppm(layer);
            }
            else
        ...
        set
        {
            if (initDone)
            {
                double sumOld = SumDoubleArray(_urea);      // original amount
                _urea = new double[dlayer.Length];          // clear any old value
                for (int layer = 0; layer < Math.Max(value.Length, dlayer.Length); ++layer)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Console.WriteLine(" Attempt to set ureappm(" + (layer + 1).ToString() + ") to a value below the lower limit, value will be set to minimum (" + ureappm_min.ToString() + ")");
                            value[layer] = ureappm_min;
                        }
                    }
    ...
    private double[] nh4ppm
    {
        get
        {
            ...
            if (initDone)
            {
                result = new double[dlayer.Length];
                for (int layer = 0; layer < dlayer.Length; ++layer)
                    result[layer] = _nh4[layer] * convFactor_kgha2ppm(layer);
            }
            else
                result = nh4ppm_reset;
            return result;
        ...
        set
        {
            if (initDone)
            {
                double sumOld = SumDoubleArray(_nh4);   // original values
                _nh4 = new double[dlayer.Length];      // clear any old value
                for (int layer = 0; layer < Math.Max(value.Length, dlayer.Length); ++layer)
                {
                    if (layer >= dlayer.Length)
                    {
                    ...
                    else
                    {
                        if (value[layer] < nh4ppm_min - epsilon)
                        {
                            Console.WriteLine(" Attempt to set nh4ppm(" + (layer + 1).ToString() + ") to a value below the lower limit, value will be set to minimum (" + nh4ppm_min.ToString() + ")");
                            value[layer] = nh4ppm_min;
                        }
                    }
    ...
    private double[] no3ppm
    {
        get
        {
            ...
            if (initDone)
            {
                result = new double[dlayer.Length];
                for (int layer = 0; layer < dlayer.Length; ++layer)
                    result[layer] = _no3[layer] * convFactor_kgha2ppm(layer);
            }
            else
                result = no3ppm_reset;
            return result;
        ...
        set
        {
            if (initDone)
            {
                double sumOld = SumDoubleArray(_no3);   // original values
                _no3 = new double[dlayer.Length];       // clear any old value
                for (int layer = 0; layer < Math.Max(value.Length, dlayer.Length); ++layer)
                {
                    if (layer >= dlayer.Length)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            Console.WriteLine(" Attempt to set no3ppm(" + (layer + 1).ToString() + ") to a value below the lower limit, value will be set to minimum (" + no3ppm_min.ToString() + ")");
                            value[layer] = no3ppm_min;
                        }
                    }
    ...
    double carbonbalance
    {
        get
        {
            ...
            double losses = 0.0;
            for (int layer = 0; layer < dlayer.Length; layer++)
                losses += dlt_c_fom_2_atm[0][layer] + dlt_c_fom_2_atm[1][layer] + dlt_c_fom_2_atm[2][layer] + dlt_c_hum_2_atm[layer] + dlt_c_biom_2_atm[layer] + SumDoubleArray(dlt_c_res_2_atm[layer]);
    ...
    private void InitCalc()
    {
        ...
        for (int layer = 0; layer < nLayers; layer++)
        {
            ...
            inert_c[layer] = finert[layer] * Soil_OC;

            // calculate microbial biomass C and N
            biom_c[layer] = MathUtility.Divide((Soil_OC - inert_c[layer]) * fbiom[layer], 1.0 + fbiom[layer], 0.0);
            biom_n[layer] = MathUtility.Divide(biom_c[layer], biom_cn, 0.0);

            // calculate C and N values for active humus
            hum_c[layer] = Soil_OC - biom_c[layer];
    ...
    private void SaveState()
    {
        ...
        dailyInitialN = oldN = SumDoubleArray(nit_tot);
        dailyInitialC = oldC = SumDoubleArray(carbon_tot);
    }
    ...
    private void DeltaState()
    {
        ...
        double dltN = SumDoubleArray(nit_tot) - oldN;
        double dltC = SumDoubleArray(carbon_tot) - oldC;
...
");
        for (int layer = 0; layer < dlayer.Length; ++layer)
        {
            ...
            layer + 1, ph[layer], OC_reset[layer], _no3[layer], _nh4[layer], _urea[layer]);
        }
        Console.WriteLine("          ------------------------------------------------");
...
");
    ...
    private void Process()
    {
        ...
        for (int layer = 0; layer < nLayers; layer++)
        {
            ...
            nh4_transform_net[layer] = dlt_nh4_decomp[layer] + dlt_n_fom_2_min[layer] + dlt_n_biom_2_min[layer] + dlt_n_hum_2_min[layer] - dlt_nitrification[layer] + dlt_urea_hydrol[layer] + no3_needed4immob;
            no3_transform_net[layer] = dlt_no3_decomp[layer] - dlt_no3_dnit[layer] + effective_nitrification[layer] - no3_needed4immob;
    ...
    //private double SumDoubleArray(carbon_tot)
    //{
    //    double SumDoubleArray(carbon_tot) = 0.0;
    //    if (dlayer != null)
    //    {
    //        int numLayers = dlayer.Length;
    //        for (int layer = 0; layer < numLayers; layer++)
    //        {
    //            SumDoubleArray(carbon_tot) += fom_c_pool1[layer] + fom_c_pool2[layer] + fom_c_pool3[layer] + hum_c[layer] + biom_c[layer];
    //        }
    //    }
    //    return SumDoubleArray(carbon_tot);
    //}

    //private double SumDoubleArray(nit_tot)
    //{
