module Registrations
   ...
   contains
    ...
    ! variables we own and make available to other modules (gettable)
         ...
         dummy = add_reg(respondToGetReg, 'esw', doubleTypeDDML, 'mm', 'Extractable soil water in each layer')
         dummy = add_reg(respondToGetReg, 'wp', doubleTypeDDML, 'mm', 'Water amount in the soil')
         dummy = add_reg(respondToGetReg, 'p', doublearrayTypeDDML, '', 'Modified psi function')
         dummy = add_reg(respondToGetReg, 'psio', doublearrayTypeDDML, 'cm', 'Soil water potential')
         dummy = add_reg(respondToGetReg, 'runoff', doubleTypeDDML, 'mm', 'Water runoff amount')
         dummy = add_reg(respondToGetReg, 'cn_runoff', doubleTypeDDML, 'mm', 'Runoff curve number')
         dummy = add_reg(respondToGetReg, 'cover_surface_runoff', doubleTypeDDML, '0-1', 'Cover affecting runoff')
         ...
         dummy = add_reg(respondToGetReg, 'flow', doublearrayTypeDDML, 'mm', 'Flow of water from each layer')
         dummy = add_reg(respondToGetReg, 'salb', doubleTypeDDML, '0-1', 'Soil surface albedo')
