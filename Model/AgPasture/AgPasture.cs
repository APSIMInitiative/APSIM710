...
public class AgPasture : Instance
{
  ...
  //component constant
    ...
    [Output][Units("MJ")] private float IntRadn;    // Intercepted Radn   

  //[Param] parameters that get initial values from the .xml    
    [Param]    private double Nspecies = 0;         //Number of species in pasture
               private int Nsp;                     //can't read in integer directly 
               float[] ftArray;                     //used for returning species values
    [Param]    private String thisCropName = "";
    [Param]    private String[] micrometType = null; 
    [Param]    private String[] speciesName = null ;   //actually species name

    [Param]    private double[] isAnnual = null;    //int: Species type (1=annual,0=perennial)
    [Param]    private double[] isLegume;	        //int:Legume (0=no,1=yes)
    [Param]    private double[] photoPath;          //int:Phtosynthesis pathways: 3=C3, 4=C4; //no consideration for CAM=5

    [Param]    private double[] dayEmerg; 	        //int:Earlist day of emergence (for annuals only)
    [Param]    private double[] monEmerg;	        //int:Earlist month of emergence (for annuals only)
    [Param]    private double[] dayAnth;	        //int:Earlist day of anthesis (for annuals only)
    [Param]    private double[] monAnth;	        //int:Earlist month of anthesis (for annuals only)        
    [Param]    private double[] daysToMature;	    //int:Days from anthesis to maturity (for annuals only)

    [Param]    private double[] cropFactor;	        //Crop Factor
    [Param]    private double[] maxResidCover;      //Maximum Residue Cover (0-1) (added to ccov to define cover)

    [Param]    private double[] dRootDepth;	        //int:Daily root growth (mm)
    [Param]    private double[] maxRootDepth;	    //int:Maximum root depth (mm)
    [Param]    private double[] rootDepth;          //int:current root depth (mm)
    [Param]    private double[] rootFnType;	        //int:Root function 0=default_1=Ritchie_2=power_law_3=proportional_depth

    [Param]    private double[] growthTmin;         //Minimum temperature (grtmin) - originally 0
    [Param]    private double[] growthTmax;         //Maximum temperature (grtmax) - originally 30
    [Param]    private double[] growthTopt;         //Optimum temperature (grtopt) - originally 12
    [Param]    private double[] growthTq;	        //Temperature n (grtemn) q -fpr curve shape-fyl
    
    [Param]    private double[] massFluxTmin;	    //grfxt1	Mass flux minimum temperature
    [Param]    private double[] massFluxTopt;	    //grfxt2	Mass flux optimum temperature
    [Param]    private double[] massFluxW0;	        //grfw1	Mass flux scale factor at GLFwater=0 (must be > 1, default = 2
    [Param]    private double[] massFluxWopt;       //grfw2	Mass flux optimum GLFwater = 0.5 water 

    [Param]   private double[] heatOnsetT;	        //onset tempeature for heat effects
    [Param]   private double[] heatFullT;	        //full temperature for heat effects
    [Param]   private double[] heatSumT;	        //temperature sum for recovery - sum of (25-mean)
    [Param]   private double[] coldOnsetT;          //onset tempeature for cold effects
    [Param]   private double[] coldFullT;	        //full tempeature for cold effects
    [Param]   private double[] coldSumT;	        //temperature sum for recovery - sum of means
    [Param]   private double[] Pm;	                //reference leaf co2 g/m^2/s maximum
    [Param]   private double[] maintRespiration;    //in % 
    [Param]   private double[] growthEfficiency;    //Pgrowth/Pgross 
    
    [Param]    private double[] satRadn;     	    //Saturated canopy radiation Pnet (MJ/m^2/day)
    [Param]    private double[] SLA;	            //Specific leaf area (m^2/kg)
    [Param]    private double[] lightExtCoeff;      //Light extinction coefficient
    [Param]    private double[] rue;
    [Param]    private double[] maxAssimiRate;      //max assimilation rate (kg/ha/day) at ref. T (20C) & daylength (12hrs);
    
    [Param]    private double[] rateLive2Dead;      //Decay coefficient between live and dead
    [Param]    private double[] rateDead2Litter;	//Decay coefficient between dead and litter
    [Param]    private double[] rateRootSen;        //Decay reference root senescence rate (%/day)
    [Param]    private double[] stockParameter;     //Stock influence parameter
    [Param]    private double[] maxRootFraction;    //maximum fraction of biomass allocated to root)
    [Param]    private double[] leafRate;           //reference leaf appearance rate without stress
    [Param]    private double[] fLeaf;	            //Fixed growth partition to leaf (0-1)
    [Param]    private double[] fStolon;	        //Fixed growth partition to stolon (0-1)
    [Param]    private double[] digestLive;         //Digestibility of live plant material (0-1)
    [Param]    private double[] digestDead;         //Digestibility of dead plant material (0-1)

    [Param]    private double[] dmtotal;            //initail shoot mass 
    ...
    [Param]    private double[] dmroot;	            //root (kg/ha)
    [Param]    private double[] dmlitter;	        //Litter pool (kg/ha)
    [Param]    private double[] dmgreenmin;	        //minimum green DM (kg/ha)     
    
    [Param]    private double[] NcleafOpt;   //=NcOptimum, all these will be input            
    [Param]    private double[] NcleafMax;	//maximum N in leaves               
    [Param]    private double[] NcleafMin;	//mimimum N in leaves               
               private double[] RemobFr;    //remoblisable fraction               
    [Param]    private double[] NcstemFr;   //stem Nc as % of leaf Nc               
    [Param]    private double[] NcstolFr;              
    [Param]    private double[] NcrootFr;   //root Nc as % of leaf Nc       
    
    [Param]    private double[] NMinFix;	//legume
    [Param]    private double[] NMaxFix;	//
    ...
    [Param]    private double[] Frgr;           // Relative growth rate factor (in most cases, Frgr=1)
                                                // not used till Mar2010
    [Param]    private double CO2ambient = 380; //ambient [CO2]
    [Param]    private double[] CO2PmaxScale;
    ...
    [Param]    public double[] kl = null;      //SW uptake parameter (/day)
    ...
    [Param]    public String NUptakeSource = "calc";
    
    [Input]    public float[] dlayer;   //Soil Layer Thickness (mm)
    [Input]    private float[] sw_dep;  //soil water by layer    
    [Input]    private float[] SAT;     //saturation point
    ...
    [Input]    private int day_of_month;
    [Input]    private int month;
    [Input]    private int year;
 ...
 [Output][Units("kg/ha")]
    ...
    [Output][Units("g/m^2")] public double stemsenescedwt { get { return StemDeadWt / 10; } }
  ...
  //** Nitrogen
    ...
    {      
        ...
        {                         
            ...
            double sum_rvlp = 0;            // note: rlvp is the root length proportion over the soil profile
            ...
            {   
                ...
                rlv[layer] = rlvp[layer] * Total_Rlength / (sum_rvlp*dlayer[layer]);
            }
            return rlv;
        }
    }
    ...
    private float SWUptakeProcess()
    {
        ...
        for (int layer = 0; layer < dlayer.Length; layer++)
        {
            SWSupply[layer] = (float)(Math.Max(0.0,
                              kl[layer] * (sw_dep[layer] - ll[layer] * (dlayer[layer]))));
