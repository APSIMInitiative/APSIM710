...
public class ApsimBuildsDB
{
    ...
    {
        string SQL = "INSERT INTO BuildJobs (UserName, Password, PatchFileName, Description, BugID, DoCommit, Status, UploadTime, linuxStatus) " +
                     "VALUES (@UserName, @Password, @PatchFileName, @Description, @BugID, @DoCommit, @Status, @UploadTime, @LinuxStatus)";
        ...
        Cmd.Parameters.Add(new SqlParameter("@UploadTime", NowString));
        if (DoCommit)
    ...
    {
        string SQL = "DELETE FROM BuildJobs WHERE ID = " + JobNumber.ToString();
        SqlCommand Command = new SqlCommand(SQL, Connection);
        Command.ExecuteNonQuery();
    }
    ...
    /// Find the next job to run.
    /// </summary>
    public DataTable GetJobs(int NumRows, bool PassOnly)
    {
        string SQL;
        if (PassOnly)
