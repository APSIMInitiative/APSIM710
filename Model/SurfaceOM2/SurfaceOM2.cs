...
using System.Text;
	...
	public partial class SurfaceOM2 : Instance
	{
		...
		class SurfaceOMGlobals
		{
			...
			//TODO - Tidy this god-awfule mess of fixed array sizes up, preferably use dictionary for holding SOMTypes
		...
		private DecompReturnStruct surfom_Pot_Decomp()
		{

			DecompReturnStruct result = new DecompReturnStruct(max_residues);
			...
			for (int residue = 0; residue < g.num_surfom; residue++)
			{
				...
				result.C_decomp[residue] = Fdecomp * sumC;
				result.N_decomp[residue] = Fdecomp * sumN;
				result.P_decomp[residue] = Fdecomp * sumP;
		...
		private void surfom_Process()
		{
			...
			surfom_set_vars(ref g.cumeos, ref leach_rain);
		...
		private void surfom_set_vars(ref float cumeos, ref float leach_rain)
		{
		...
		private void surfom_Send_PotDecomp_Event()
		{
			...
			for (int residue = 0; residue < g.num_surfom; residue++)
			{
				...
				SOMDecomp.Pool[residue].FOM.amount = divide(d.C_decomp[residue], c.C_fract[residue], 0.0f);
				SOMDecomp.Pool[residue].FOM.C = d.C_decomp[residue];
				SOMDecomp.Pool[residue].FOM.N = d.N_decomp[residue];
				SOMDecomp.Pool[residue].FOM.P = d.P_decomp[residue];
