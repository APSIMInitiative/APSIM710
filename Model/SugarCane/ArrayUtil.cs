...
using CSGeneral;
    ...
    //Make converting between Fortran (One based) arrays and DotNet (Zero based) easier.
                ...
                if (X <= X_cord[indx])
                    ...
                    if (indx == si)
                        ...
                        if (mu.reals_are_equal(X, X_cord[indx]))
                            ...
                            {
                            //! interpolate - y = mx+c
                            m = MathUtility.Divide((Y_cord[indx] - Y_cord[indx - 1]), (X_cord[indx] - X_cord[indx - 1]), 0.0);   //m = dy/dx  //there are different slopes between any two points in the array
