...
class InsertFilesIntoSetup
{
    ...
    private XmlNode ReadObjectIntoXml(StreamReader In, XmlNode ParentNode, string ObjName)
    {
        ...
        while (Line != "}")
        {
            ...
            if (LineBits.Count == 2)  // keyword = value
            {
                // Strip off a leading and trailing " chars.
                LineBits[0] = LineBits[0];
                LineBits[1] = LineBits[1];
                XmlHelper.SetValue(ObjNode, LineBits[0], LineBits[1]);
            }
            else if (LineBits.Count == 1 && Line.Contains(':')) // object ref
            {
                String[] ObjectBits = Line.Split(":".ToCharArray(),
                                               StringSplitOptions.RemoveEmptyEntries);
                if (ObjectBits.Length != 2)
                    throw new Exception("Unknown type of line: " + Line);

                XmlNode ChildNode = ReadObjectIntoXml(In, ObjNode, "Object");

                string ChildObjName = ObjectBits[0].Replace("\"", "").Replace("{", "").Replace("}", "");
                string ID = ObjectBits[1].Replace("\"", "").Replace("{", "").Replace("}", "");
